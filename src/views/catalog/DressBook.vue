...
<template>
  <div v-if="success" class="text-green-500 text-sm">
    {{ $i18n.t("rent.dress_booking_save_success") }}
    {{ $i18n.t("rent.dress_booking_save_success_send_to_messenger") }}:
    <div class="block">
      <br />
      <a
        :href="
          ...
          '/' +
          `?text=${frontURL}/${$i18n.locale}/dress/${dress_id}%0A` +
      ...
      >
        {{
          $i18n.t("rent.dress_booking_save_success_send_to_messenger_button")
        }}
      </a>
    </div>
  </div>
  <div v-else="success">
    <div class="mb-3 text-sm">
      {{ $i18n.t("rent.dress_booking_form_title") }}:
    </div>
    <FormErrors :error="errors.dress_id" />

    <div class="mt-5 text-sm">
      <FormErrors :error="errors.quantity" />
      {{ $i18n.t("rent.dress_booking_quantity") }}:
      <input
        ...
        :placeholder="$i18n.t('rent.dress_booking_quantity')"
        required=""
        min="1"
        max="100"
      />
      {{ $i18n.t("rent.dress_booking_quantity_available") }}:
      <span :class="date ? 'text-green-500' : 'text-red-500'">
        {{
          this.date
            ? this.bookings.find((item) => item.date === this.date).booking[0]
                .free
            : $i18n.t("rent.dress_booking_quantity_select_date") + ":"
        }}
      </span>
    </div>
    ...
    <div class="mt-5">
      ...
      <div class="mb-2">
        <input
          ...
          :placeholder="$i18n.t('rent.dress_booking_enter_email')"
          required
        />
      </div>
      ...
      <div class="mb-2">
        <input
          ...
          id="phone_number"
          class="block p-2 w-70 rounded-md border-1 bg-gray-0 border-gray-300 text-gray-900 text-sm w-half hover:bg-gray-50 hover:text-gray-700 focus:bg-gray-50"
          :placeholder="$i18n.t('rent.dress_booking_enter_phone_number')"
          required
        />
      </div>
    </div>
    <div class="mt-5 <sm:mt-4 flex gap-4 <sm:justify-center">
      ...
      >
        {{ $i18n.t("rent.dress_booking_button_title") }}
      </button>
    </div>
  </div>
</template>
